...
TG Vault 提供两层 Telegram 能力：**Bot 基础能力**和可选的**账号级下载器**。二者都在 Web 的 **设置 → Telegram** 中配置。
...
在 **设置 → Telegram → Telegram Bot 用户权限** 填写允许使用 Bot 的数字 user ID。用户可通过 `@userinfobot` 查看自己的 ID。
...
- 如果启用后 session 未就绪，任务会明确失败。

账号必须已经加入要读取的频道或群组，并具有查看历史消息的权限。
...
切换到“仅保留错误”会立即压缩已有的成功/跳过明细。“删除任务历史”、临时文件清理和删除实体文件是三类不同操作。

两个并发参数可在 **设置 → 维护 → 高级任务设置** 中调整：同时处理的文件数量，以及单个文件的分片数量。
...
[返回文档中心](./) · [快速部署](./quick-start.html) · [了解任务架构](./architecture.html)
