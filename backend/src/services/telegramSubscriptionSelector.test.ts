...
const findSubscription = source.slice(source.indexOf('export async function findTelegramSubscription'), source.indexOf('export async function unsubscribeTelegramChannel'));
...
assert.match(findSubscription, /resolveUniqueTelegramSubscriptionId/);
assert.match(unsubscribe, /findTelegramSubscription/);
