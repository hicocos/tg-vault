...
<p class="doc-lead"><strong>把 Telegram 变成你的自动化私有云入口。</strong><br>从 Telegram 或 Web 接收内容，转存到本地磁盘或多个云存储账户，并在同一个 Web 控制台中检索和管理。</p>

<div class="callout">
本网站记录 <strong>TG Vault 当前项目</strong>的部署与使用方法。当前版本支持安装脚本快速部署、Web 管理 Telegram Bot 凭据、网页登录账号级下载器、统一任务中心、多存储账户以及完整备份恢复。
</div>
...
- **统一任务中心**：集中查看 Web 上传、Telegram 和频道任务，并安全取消、重试或移除终态记录。
...
- Web 上传和 Telegram 下载都经过部署 TG Vault 的服务器，需要足够的临时磁盘、带宽和内存。
